...
export const ReadTool = Tool.define("read", async (initCtx) => {
  ...
  if (isToolAllowed("grep")) {
    suggestions.push("- Use the grep tool to find specific content in large files or files with long lines.")
  }
  ...
  return {
    description: DESCRIPTION.replace("${READ_TOOL_SUGGESTIONS}", suggestions.join("\n")),
    parameters,
